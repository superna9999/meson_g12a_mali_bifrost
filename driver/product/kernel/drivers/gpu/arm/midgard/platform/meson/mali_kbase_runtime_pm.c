...
static int gpu_power_on(struct kbase_device *kbdev, uint32_t  mask)
{
	...
	kbase_reg_write(kbdev, GPU_CONTROL_REG(TILER_PWRON_LO), 0xffffffff, NULL);
	kbase_reg_write(kbdev, GPU_CONTROL_REG(TILER_PWRON_HI), 0xffffffff, NULL);

	/* Power On all L2 cache */
	kbase_reg_write(kbdev, GPU_CONTROL_REG(L2_PWRON_LO), 0xffffffff, NULL);
	kbase_reg_write(kbdev, GPU_CONTROL_REG(L2_PWRON_HI), 0xffffffff, NULL);

	/* Power On all Shader Cores */
	kbase_reg_write(kbdev, GPU_CONTROL_REG(SHADER_PWRON_LO), 0x3, NULL);
	kbase_reg_write(kbdev, GPU_CONTROL_REG(SHADER_PWRON_HI), 0x0, NULL);
